...
var (
	...
	cacheZhgozeroArticleArticleIdPrefix = "cache:zhgozeroArticle:article:id:"
)
...
func (m *defaultArticleModel) Delete(ctx context.Context, id int64) error {
	zhgozeroArticleArticleIdKey := fmt.Sprintf("%s%v", cacheZhgozeroArticleArticleIdPrefix, id)
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, id)
	}, zhgozeroArticleArticleIdKey)
	return err
}
...
func (m *defaultArticleModel) FindOne(ctx context.Context, id int64) (*Article, error) {
	zhgozeroArticleArticleIdKey := fmt.Sprintf("%s%v", cacheZhgozeroArticleArticleIdPrefix, id)
	var resp Article
	err := m.QueryRowCtx(ctx, &resp, zhgozeroArticleArticleIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
...
func (m *defaultArticleModel) Insert(ctx context.Context, data *Article) (sql.Result, error) {
	zhgozeroArticleArticleIdKey := fmt.Sprintf("%s%v", cacheZhgozeroArticleArticleIdPrefix, data.Id)
...
func (m *defaultArticleModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheZhgozeroArticleArticleIdPrefix, primary)
}
